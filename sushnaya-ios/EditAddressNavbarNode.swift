...
protocol EditAddressNavbarDelegate: class {
...
class EditAddressNavbarNode: ASDisplayNode {
    ...
    weak var delegate: EditAddressNavbarDelegate?
...
extension EditAddressNavbarNode: SegmentedControlDelegate {
    func segmentedControl(segmentedControl: SegmentedControlNode, didSelectSegment segment: Int) {
        switch  segment {
        case 0:
            self.delegate?.editAddressNavbarDidTapMapButton(node: self)
        case 1:
            self.delegate?.editAddressNavbarDidTapFormButton(node: self)
        default:
            break
        }
    }
